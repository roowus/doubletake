...
function line(e: RunEvent): string {
  ...
  switch (e.type) {
    ...
    case 'tool_call':
      return `call ${String(p.tool ?? p.name ?? 'tool')} ${p.input ? JSON.stringify(p.input).slice(0, 160) : ''}`;
    case 'tool_result':
      return `result ${String(p.tool ?? p.name ?? 'tool')} ${p.summary ? String(p.summary).slice(0, 160) : ''}`;
    case 'text':
      return String(p.text ?? '').slice(0, 300);
    case 'error':
      return `error ${String(p.message ?? p.error ?? JSON.stringify(p))}`;
    case 'done':
