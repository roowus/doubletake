...
| T14 | Injected answer text carries HTML/SVG/diagram text that runs script or exfiltrates when the chat is opened | Markdown renders with raw HTML off. Fenced `svg` is inlined only after DOMPurify's SVG profile removed scripts, event handlers, links, styles, images, `use`/`foreignObject` and external `url()` refs; fenced `mermaid` renders at `securityLevel: 'strict'` with HTML labels off and its SVG goes through the same profile (styles allowed, remote `url()`/`@import` stripped); fenced `chart` is drawn by our renderer from zod-validated numbers and short labels, never from markup (`apps/web/src/svg.ts`, `components/{Markdown,Mermaid,Chart}.tsx`, all tested) | A DOMPurify bypass; the picture itself can still mislead, which is why the prompt forbids facts that live only in the diagram |
