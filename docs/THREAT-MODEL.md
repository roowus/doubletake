...
| T14 | Injected answer text carries HTML/SVG that runs script or exfiltrates when the chat is opened | Markdown renders with raw HTML off; only a fenced `svg` block is inlined, and only after DOMPurify's SVG profile removed scripts, event handlers, links, styles, images, `use`/`foreignObject` and external `url()` refs (`apps/web/src/components/Markdown.tsx`, tested) | A DOMPurify bypass; the picture itself can still mislead, which is why the prompt forbids facts that live only in the diagram |
