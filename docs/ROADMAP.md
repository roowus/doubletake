...
Verified on a Galaxy S25 FE (2026-09-03) and, for pairing plus FCM delivery, on a Pixel 4 XL over `adb reverse` (2026-09-04) and, for notification tap-to-open with the app killed, on an API 36 emulator (2026-09-06): pairing (code and QR URL), share sheet for URL and
...
## M5 — More brains — done 2026-09-04 (all six adapters/presets verified live)

- ✅ `headless-cli` adapter with presets for `claude -p`, Codex, Gemini CLI, OpenCode, Hermes
  (unit-tested with a fake `spawn`; all five run live with resume through the local router,
  2026-09-04: codex-cli 0.153.4, Gemini CLI 0.58.0, OpenCode 1.18.29, Hermes 0.21.0).
...
~~Telegram and ntfy notification channels~~ (done 2026-09-04, [ADR 0019](adr/0019-owner-notification-channels.md)) · ~~iOS share extension~~ (done 2026-09-04, photo/video upload 2026-09-06, simulator-verified, [ADR 0027](adr/0027-ios-share-extension.md); **unverified** on a device) · ~~tailscale multi-device
